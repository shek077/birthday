...
const App: React.FC = () => {
  ...
  useEffect(() => {
    if (isTyping && wish) {
      let i = 0;
      setDisplayedWish(''); // Reset before starting
  ...
  if (showSecretMessage) {
    return (
      <div className="relative min-h-screen w-full flex flex-col items-center justify-center p-6 bg-black text-center z-[100] animate-in fade-in zoom-in duration-500 overflow-y-auto">
        ...
        <div className="flex gap-4 text-5xl md:text-8xl mb-6 animate-bounce">
          <span>🍗</span>
          <span>🍚</span>
          <span>🍗</span>
        </div>
        
        <h2 className="text-xl md:text-6xl font-black text-transparent bg-clip-text bg-gradient-to-r from-orange-400 via-yellow-200 to-red-500 mb-6 leading-tight drop-shadow-[0_0_20px_rgba(251,146,60,0.5)] px-2 uppercase break-words max-w-2xl">
          MARIYAATHIKK MANDHI VAANGHI THANNOLANAM, KEETADI MAAKIRI 🍗✨
        </h2>

        <p className="text-base md:text-2xl text-orange-200 font-bold mb-10 italic opacity-80 px-4">
        ...
        <div className="mt-8 text-4xl opacity-20 animate-pulse">🏃‍♀️💨 🍖</div>
      </div>
    );
  }

  return (
    <div className={`relative min-h-screen w-full flex flex-col items-center bg-[#1a0515] overflow-x-hidden selection:bg-pink-500 selection:text-white ${isOpened ? 'justify-start' : 'justify-center'}`}>
      <style>{`
        .custom-scrollbar::-webkit-scrollbar { width: 4px; }
        .custom-scrollbar::-webkit-scrollbar-track { background: rgba(255, 255, 255, 0.05); }
        .custom-scrollbar::-webkit-scrollbar-thumb { background: #ec407a; border-radius: 10px; }
        .text-glow { text-shadow: 0 0 12px rgba(236, 64, 122, 0.4); }
      ...
      {/* The Moon */}
      <div className="fixed top-4 right-4 md:top-10 md:right-10 pointer-events-none z-0">
        <div className="relative w-12 h-12 md:w-24 md:h-24 rounded-full bg-[#fffcf0] shadow-[0_0_40px_rgba(255,252,240,0.4)] animate-pulse overflow-hidden opacity-60">
          <div className="absolute top-2 left-3 w-2 h-2 rounded-full bg-gray-200/50" />
          <div className="absolute bottom-3 right-5 w-4 h-4 rounded-full bg-gray-200/50" />
        </div>
      </div>

      {!isOpened ? (
        <div className="flex flex-col items-center text-center z-10 space-y-8 px-4 py-10 w-full max-w-lg animate-in fade-in duration-700">
        ...
        <div className="w-full max-w-4xl flex flex-col items-center space-y-8 md:space-y-12 animate-in fade-in zoom-in duration-1000 z-10 px-4 py-12 pb-32">
          <Confetti />
          ...
          <div className="text-center space-y-2">
            ...
            <h1 className="text-3xl md:text-7xl font-black text-transparent bg-clip-text bg-gradient-to-b from-yellow-200 via-yellow-100 to-yellow-500 drop-shadow-[0_5px_15px_rgba(255,215,0,0.3)] tracking-tight leading-tight px-2">
              HAPPY BIRTHDAY HARITHA! 💖
            </h1>
          </div>

          <div className="w-full bg-white/5 backdrop-blur-2xl rounded-3xl md:rounded-[3rem] p-6 md:p-12 border border-white/10 shadow-2xl relative">
            <div className="max-h-[50vh] overflow-y-auto custom-scrollbar px-1">
               <p className="text-lg md:text-3xl font-black italic text-pink-50 leading-relaxed text-center drop-shadow-md break-words text-glow">
                {displayedWish ? `"${displayedWish}"` : (
                  <span className="flex flex-col items-center gap-4 py-10">
                    <span className="animate-pulse text-pink-300/60">Gathering magical stardust... ✨</span>
          ...
          <div className="flex flex-col items-center gap-8 py-4 w-full">
            <div className="flex gap-4 md:gap-10 text-4xl md:text-6xl">
            ...
            <div className="text-center space-y-6">
              ...
              <div 
                ...
                className={`text-7xl md:text-9xl cursor-pointer select-none transition-all duration-[1s] relative inline-block ${cakeEaten ? 'scale-0 opacity-0 -translate-y-20' : 'hover:scale-110 active:scale-90'}`}
              >
            ...
            {cakeEaten && (
              <div className="animate-in zoom-in slide-in-from-bottom duration-700 text-xl md:text-4xl font-black text-yellow-300 flex flex-col items-center gap-3 px-4 text-center">
                <span className="drop-shadow-lg uppercase tracking-wide">MAGICAL! YOU'RE THE BEST! 🍭</span>
                <span className="text-xs md:text-lg font-bold text-pink-200 tracking-[0.2em] bg-pink-900/40 px-8 py-3 rounded-full border border-pink-500/20 uppercase">May all your dreams come true! 🌙</span>
              </div>
            )}
          </div>
          ...
          <div className="w-full flex justify-center pt-8">
            <button 
              ...
              className="w-full max-w-xs md:max-w-md px-8 py-5 bg-gradient-to-r from-pink-600 via-rose-500 to-orange-400 rounded-full font-black text-lg md:text-xl transition-all shadow-[0_10px_40px_rgba(225,29,72,0.4)] hover:shadow-pink-500/60 hover:scale-105 active:scale-95 border-2 border-white/20 uppercase tracking-widest text-white"
            >
              RELIVE THE MAGIC 🌌
            </button>
          </div>
      ...
      <footer className="w-full py-8 mt-auto text-pink-400/30 text-[9px] md:text-xs font-black tracking-[0.4em] uppercase pointer-events-none text-center px-4">
